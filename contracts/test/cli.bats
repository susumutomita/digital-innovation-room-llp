...
@test "engagement:status prints mapped status and links" {
  run bash -c "./cli/engagement.sh engagement:status"
  [ "$status" -eq 0 ]
...
@test "engagement:set-split PRETTY=1 trims whitespace and prints Total BPS" {
  run bash -c "PRETTY=1 ./cli/engagement.sh engagement:set-split '0xA, 0xB' '5000, 5000'"
  [ "$status" -eq 0 ]
...
@test "cast_send prints tx explorer link" {
  run bash -c "./cli/engagement.sh engagement:lock"
  [ "$status" -eq 0 ]
